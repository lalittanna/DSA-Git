...
public class ThreeSum3 {
    ...
    */
    public static Set<List<Integer>> getTriplets(int[] nums, int target){
        ...
        getAllTriplets(subsets, nums, list, 0, target);
    ...
    public static void getAllTriplets(Set<List<Integer>> set, int[] nums, List<Integer> list, int start, int target){
        if (list.size() == 3){
        ...
        for (int i = start; i < nums.length && nums[i] <= target; i++){
            ...
            getAllTriplets(set, nums, list, i + 1, target - nums[i]);
